...
suspend fun storeTaskInvoke(
    ...
    return try {
        val response: Response<ApiResponse> = RetrofitInstance.calendarApi.addTask(request)
...
suspend fun deleteTaskInvoke(
    ...
    return try {
        val response: Response<ApiResponse> = RetrofitInstance.calendarApi.deleteTask(request)
        if (response.isSuccessful) {
            response.body()?.let { data ->
                Result.success(value = data)
            }
...
suspend fun getTasksInvoke(
    ...
    return try {
        val response: Response<TaskResponse> = RetrofitInstance.calendarApi.getTasks(request)
